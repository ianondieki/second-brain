# Legacy: n8n + Docker stack (archived)

**Unmaintained, kept for reference.** Nothing in this folder is built, tested or supported. The current tools are the
local project reminders (`reminder/`, email + WhatsApp) and the WhatsApp voice adviser (`adviser/`), documented in the
[root README](../README.md), [`docs/10-project-reminders.md`](../docs/10-project-reminders.md) and
[`docs/11-whatsapp-voice-adviser.md`](../docs/11-whatsapp-voice-adviser.md). The hosted platform is specified in
[`docs/spec/`](../docs/spec/).

Moved here in Phase 1 (REQ-FND-01) with one `git mv`: `n8n/`, `docker-compose.yml`, `notes/` (Obsidian templates),
`scripts/smoke-test.*`, `scripts/test-pipeline.mjs`, `docs/01`–`09` and `docs/10-evening-checkin.md`. Paths below are
relative to this folder. The environment template this stack used is kept as [`legacy/.env.example`](.env.example).
