...
| D | Email bombing through signup, magic-link, resend or unverified-login routes | One ledger (`login_attempts`, HMAC digests): 3 emails per (address, IP) and 6 per address from any IP per 15 min, 300 per IP; "account exists" at most once a day; drops are silent to the caller and logged (`auth.email_throttled`) | Medium; an attacker with several IPs can use up an address's 6 emails per 15 min and delay a real magic link (password login unaffected) | `::test_logins_to_an_unverified_account_do_not_flood_its_inbox`, `::test_a_fourth_signup_from_one_ip_sends_no_email` |
...
| E | Staff `support` escalates via impersonation | Impersonation off by default, audited, owner notified; read-only role (`docs/spec/03`) | Low | AC-ADM-3 |
