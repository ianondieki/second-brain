...
| I | Credentials in the URL: a form submitted before the page's JavaScript loads falls back to GET (`/login?email=…&password=…`), reaching history, proxy logs and Referer | Every credential or code form uses `components/ui/Form`: `method="post"` and a submit button disabled until hydration, so neither a click nor Enter submits natively (T1.9 UX review BLOCKER) | Low | `frontend/components/ui/Form.test.tsx`; `frontend/e2e/auth.spec.ts` (JavaScript disabled) |
| S | Cookie-header injection on the web server's call to the API: a planted bare-name cookie whose decoded value carries `; __Host-bridge_session=…` renders signed-in pages as the attacker (login CSRF) | `lib/api/server.ts` reads one cookie name chosen by the web server's `COOKIE_SECURE` (no fallback) and forwards only values in the API token alphabet (T1.9 security review MAJOR) | Low | `frontend/lib/api/cookies.test.ts` |
| D | A planted malformed CSRF cookie makes the web client throw before any request, blocking login | An undecodable cookie counts as absent, so the client fetches a fresh token | Low | `frontend/lib/api/client.test.ts` |
| T | Clickjacking or UI redress of the auth screens | Every web page sends `X-Frame-Options: DENY`, `X-Content-Type-Options: nosniff`, `Referrer-Policy`, `Permissions-Policy` and a report-only CSP baseline with `frame-ancestors 'none'`; enforced nonce CSP and HSTS arrive with Caddy (Phase 8, REQ-SEC-03) | Low; CSP is report-only until Phase 8 | `frontend/security-headers.test.ts`, `frontend/e2e/auth.spec.ts` |
| S | Open redirect through an API-supplied URL (402 upgrade path) | The client resolves the URL against its own origin and accepts only that origin; tab, CR and LF are refused | Low | `frontend/lib/api/errors.test.ts` |
| I | Magic-link token left in the browser after sign-in (address bar, history, Next's router state, navigation timing) | The fragment is scrubbed from the address bar, `history.state` and Next's router URL; the page sends no Referer; `/auth/link` loads scripts only from `/_next/static`, so no telemetry can read the navigation-timing entry that keeps the fragment | Low; any future telemetry must stay off `/auth/link` | `frontend/app/(public)/auth/link/router-sync.test.tsx`, `frontend/e2e/auth.spec.ts` |
| I | A remembered email address survives sign-out on a shared browser | `sessionStorage` key removed on sign-out and after a link sign-in | Low | `frontend/components/SignOutButton.test.tsx`, `frontend/app/(public)/auth/link/LinkSignIn.test.tsx` |
